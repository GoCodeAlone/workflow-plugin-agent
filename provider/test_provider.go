...
// TestProvider implements Provider by delegating to a ResponseSource.
...
func (tp *TestProvider) Stream(ctx context.Context, messages []Message, tools []ToolDef) (<-chan StreamEvent, error) {
	...
	ch := make(chan StreamEvent, 2+len(resp.ToolCalls))
	if resp.Content != "" {
		ch <- StreamEvent{Type: "text", Text: resp.Content}
	}
	for i := range resp.ToolCalls {
		ch <- StreamEvent{Type: "tool_call", Tool: &resp.ToolCalls[i]}
	}
